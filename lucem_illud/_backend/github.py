...
tokenFile = '../token.txt'
orgName = 'UChicago-Computational-Content-Analysis'
repoName = 'Homework-Notebooks'
...
def getStudentRepo(ghName, outputName, auth = None, name = repoName):
    ...
    try:
        ...
        try:
            repoURL = "/repos/{}/Homework-Notebooks".format(ghName, name)
            repoDat = getGithubURL(repoURL, auth = auth)
        except RuntimeError as e2:
            try:
                repoURL = "/repos/{}/UChicago-Computational-Content-Analysis".format(ghName, name)
                repoDat = getGithubURL(repoURL, auth = auth)
...
def makeStudentRepo(targetDir = '.', name = repoName):
    ...
    data = {
        "name": name,
        "description": 'Assignments for Computational Content Analysis',
        "homepage": "https://github.com/UChicago-Computational-Content-Analysis",
        "private": False,
        "has_issues": True,
        "has_projects": False,
        "has_wiki": False,
